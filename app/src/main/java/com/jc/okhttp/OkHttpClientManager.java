...
import com.jc.bean.RequestBean;
...
public class OkHttpClientManager {
    ...
    public GetDelegate getGetDelegate() { return mGetDelegate; }
    ...
    public static DisplayImageDelegate getDisplayImageDelegate() { return getInstance()._getDisplayImageDelegate(); }

    public static DownloadDelegate getDownloadDelegate() { return getInstance()._getDownloadDelegate(); }
    ...
    public static void httpAsyn(final RequestBean requestBean, final ResultCallback asyncallback){
            if (requestBean == null)
                throw new NullPointerException("RequestBean is Null");
            if (asyncallback == null)
                throw new NullPointerException("ResultCallback is Null");

            final Map<String,String> datas =  requestBean.datas;
            final StringBuilder url = new StringBuilder(requestBean.url);
            if ("GET".equals(requestBean.method)){
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD){
                    getInstance().getGetDelegate().getAsyn(url.toString().toString(), asyncallback);
                }else {
                    final Handler handler = new Handler(new Handler.Callback() {
                        ...
                        public boolean handleMessage(Message msg) {
                            switch (msg.what) {
                                case 1:
                                    String responseText = msg.getData().getString("result");
                                    if (asyncallback!= null)
                                        asyncallback.onResponse(responseText);
                            }
                            return true;
                        }
                    });
        ...
        }else if("POST".equals(requestBean.method)){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD){
                    getInstance().getPostDelegate().postAsyn(url.toString(),datas,asyncallback);
                }else {
                    ...
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            Set<String> keySets = datas.keySet();
                            String[] keys = (String[]) keySets.toArray();
                            String[] values = new String[keys.length];
                            for (int i = 0;i<keys.length;i++){
                                values[i] = datas.get(keys[i]);
                            }
                            String result = request(requestBean.url,
                                    keys,
                                    values
                                    );
                            Log.e("result=",result);
    ...
    public static String httpURLPost(RequestBean requestBean){
...
//            HttpsURLConnection.setDefaultHostnameVerifier(hostnameVerifier);
    ...
    public static String getData(String sendUrl){
        String  s = "";
        try{
